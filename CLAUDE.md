...
### 2026-08-03 (noche, cont.) — Calibración de precisión (spec Parte 9)
- Nota visible agregada al inicio de `docs/spec.md` explicando por qué se descartó la función de foto — sin borrar ni reescribir el spec original (sigue siendo el documento ya calificado).
- Construida la calibración de precisión que faltaba: al marcar una tarea como hecha en "Tu plan", aparece "¿Cómo te fue con el tiempo?" con 3 botones (Más rápido / Como se esperaba / Más lento) — "sin cronómetro", como pide el spec.
- Nueva tarjeta "Precisión de estimación" en KPI: muestra "Calibrando… X/15" hasta tener 15 capturas o 5 días de uso; después, el % de error promedio de las últimas 15 (ventana móvil).
- Recalibración: cada 15 capturas, si hay sesgo sistemático (>10% en cualquier dirección), se ajusta `factorAdjust` (±0.03 por ronda, acotado a ±0.15) y se suma al factor de energía de todos los bloques.
- Verificado con prueba real (34 capturas simuladas, no solo revisión de código): el conteo escala de 1 en 1 sin fallar, el desbloqueo a los 15 funciona, y el sesgo sistemático con "Más lento" repetido subió `factorAdjust` de 0 a 0.03 como se esperaba.
